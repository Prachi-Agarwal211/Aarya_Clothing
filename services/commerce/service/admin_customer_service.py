    ...
    def get_customers_by_segment(self, segment: str) -> List[Dict[str, Any]]:
        ...
        if segment == "new":
            ...
            customers = [
                ...
                if len(self.db.query(Order).filter(Order.user_id == c.id).all()) > 2
            ]
        
        elif segment == "vip":
            # Customers who spent more than 10000
            customers = []
            for c in self.db.query(User).filter(User.role == UserRole.CUSTOMER).all():
                total_spent = sum(
                    float(o.total_amount)
                    for o in self.db.query(Order).filter(Order.user_id == c.id).all()
                )
            ...
            customers = [
                c for c in customers
                if not self.db.query(Order).filter(
                    Order.user_id == c.id,
